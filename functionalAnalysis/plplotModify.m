function h=plplotModify(x, xmin, alpha)
...
switch f_dattype,
    ...
    case 'REAL',
        ...
        % h(2) = loglog(cf(:,1),cf(:,2),'k--','LineWidth',4); 
        h(1) = loglog(c(:,1),c(:,2),'LineWidth',2,'color','r');
        hold off;
        ...
        set(gca,'XLim',abs(xr),'XTick',10.^xrt);
        set(gca,'YLim',abs(yr),'YTick',10.^yrt,'FontSize',16);
    ...
    case 'INTS',
        ...
        h(1) = loglog(c(:,1),c(:,2),'bo','MarkerSize',8,'MarkerFaceColor',[1 1 1]); hold on;
        % h(2) = loglog(cf(:,1),cf(:,2),'k--','LineWidth',4); 
        h(1) = loglog(c(:,1),c(:,2),'LineWidth',8,'color','r'); 
        hold off;
        %%% xr  = [10.^floor(log10(min(x))) 10.^ceil(log10(max(x)))];
        %%% modify line 84 to line 87.
        %%% min(x)-->min(x+1) avoid the case of "log10(0) = infinitesimal".
        xr  = [10.^floor(log10(min(x+1))) 10.^ceil(log10(max(x)))];
        xrt = (round(log10(xr(1))):2:round(log10(xr(2))));
        if length(xrt)<4, xrt = (round(log10(xr(1))):1:round(log10(xr(2)))); end;
        yr  = [10.^floor(log10(1/n)) 1];
        yrt = (round(log10(yr(1))):2:round(log10(yr(2))));
